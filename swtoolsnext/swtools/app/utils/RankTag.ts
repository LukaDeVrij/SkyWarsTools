...
export function getPlayerRank(playerOverall: OverallResponse, onlyPackages = false): PlayerRank {
	...
	if (playerDisplay.monthlyRankColor || playerDisplay.rankPlusColor) {
		const customRankColor = MinecraftFormatting[playerDisplay.monthlyRankColor as keyof typeof MinecraftFormatting];
		const customPlusColor = MinecraftFormatting[playerDisplay.rankPlusColor as keyof typeof MinecraftFormatting];
		if (customRankColor) {
